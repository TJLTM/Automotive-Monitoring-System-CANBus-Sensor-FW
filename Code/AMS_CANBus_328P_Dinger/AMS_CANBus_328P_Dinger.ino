...
long PacingTimer;
#define DeviceType 6
#define MaxInputChannelNumber 3
#define MaxOutputChannelNumber 3
const int InputPins[] PROGMEM = { D3, D4, D2 };
const int OutputPins[] PROGMEM = { D5, D6, D9 };
const int SensorType PROGMEM = 6;
...
void UnitsABRResponse(bool FromSerial, int Channel) {
  ...
  */
    if (FromSerial == false) {
      CanBusSend(DeviceAddress, 3, 0x07, byte("R"), Channel, 0x09, 0x00, 0x00, 0x00, 0x00);
    } else {
      SendSerial("UnitABR:0x07:" + String(Channel) + ":" + String(0x09));
    }
  } else {
    SetError(3, 0x07);
  }
...
//Sensor Helpers
//----------------------------------------------------------------------------------------------------
bool InputChannelRangeCheck(int Channel) {
  if (Channel >= 0 && Channel <= MaxInputChannelNumber) {
    return true;
  } else {
    return false;
  }
...
bool OutChannelRangeCheck(int Channel) {
  if (Channel >= 0 && Channel <= MaxOutputChannelNumber) {
    return true;
  } else {
    return false;
  }
